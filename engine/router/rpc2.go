...
func NewRPCCallMgr(reqmgr requester) (*RPCCallMgr, error) {
	...
	r := &RPCCallMgr{
		...
		calls:         make(map[structs.NRoute]*RPCCall2),
	}

	for _, route := range reqmgr.Routes() {
		rpccall, err := newRPCCall2(r, route)
		if err != nil {
...
// RPCCall2 manages the data for each RPC call.
type RPCCall2 struct {
	rpc rpcmanager
	adp AdpRPCer
...
// String returns a representation of an RPCCall2 instance.
func (r RPCCall2) String() string {
	ls := new(common.LogString)
	ls.AddF("RPCCall2 - %d\n", r.id)
	ls.AddF("Service: %v\n", r.rpc.service())
...
func newRPCCall2(rpcmgr rpcmanager, route structs.NRoute) (*RPCCall2, error) {
	r := &RPCCall2{
		id:    getRPCID(),
		rpc:   rpcmgr,
		route: route,
...
func (r *RPCCall2) setSent() {
	r.Lock()
	defer r.Unlock()
	r.sent = true
}

func (r *RPCCall2) prepRPC() (rqstCopy interface{}, err error) {
